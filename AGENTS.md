...
- **`_workspace` Injection**: The engine automatically adds a `_workspace` key to the `params` dictionary passed to `execute()`, providing the absolute path to the persistent workspace directory on the host.
...
2. The `Engine` scans this directory after setiap tool execution.
